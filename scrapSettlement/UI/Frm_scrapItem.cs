...
{
    public partial class Frm_scrapItem : Form
    {
        public Frm_scrapItem()
        {
            InitializeComponent();
            this.initialize();
        ...
        private void Tsb_add_Click(object sender, EventArgs e)
        {
            ...
            using (var db = new ScrapSettleContext())
            {
                ...
                var Query = from s in db.Scraps.AsNoTracking()
                ...
                {
                    maxCusCode = Convert.ToInt32(Query.Max()) + 1;
                }
        ...
        private void tsb_delete_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                if (DialogResult.Yes == MessageBox.Show("是否确定删除", "删除提醒", MessageBoxButtons.YesNo, MessageBoxIcon.Warning))
                {
                    ...
                    List<Scrap> del = (from d in db.Scraps
                                                  where d.ScrapCode ==selected
                                                  select d).ToList<Scrap>();
                    //移除数据库的数据
                    db.Scraps.Remove(del[0]);
                    db.SaveChanges();
                    clearDate();
                    ...
                    if (saveOrModifQueryFlag == saveOrChangeOrQueryMolde.save.ToString())
                    {
                        ...
                        List<Scrap> scraps = scrapList.Where(c => c.ScrapCode == System.Convert.ToInt32(selected)).ToList<Scrap>();
                        scrapList.Remove(scraps[0]);
        ...
        private void bind_gv_dateSource()
        {
            ...
            {
                this.dataGridView1.DataSource = new ScrapSettleContext().Scraps.ToList<Scrap>();
            }
            //新增状态的数据源
            else
            {
                this.dataGridView1.DataSource = scrapList;
            }
